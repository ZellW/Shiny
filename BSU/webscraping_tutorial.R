...
# This code work for static web pages.  RSelenium can be used for JavaScript Websites - a bit more challenging
# FYI - RCrawler
...
setwd("~/GitHub/R_Programming/BSU")
...
# Skipped Womens, Kids and Gear Product Categories
road_bikes_tbl <- tibble(
    product_family = c("Endurance Road", "Elite Road", "Gravel - All Road", "Cyclocross", "TT and TRI"),
    ...
    add_column(category = "Road", .before = 1) #Did not know the option to put the new column where you want

road_bikes_tbl

mountain_bikes_tbl <- tibble(
    product_family = c("Cross Country", "Trail", "All Mountain", "Enduro", "Sport", "Fat Bike"),
    nid = c(
...
bike_category_nids_tbl <- bind_rows(
    ...
    mutate(url = str_glue("https://www.cannondale.com/en/USA/Products/ProductCategory.aspx?nid={nid}") )
...
#Use Google Inspector to find that the .relatedProducts class along with the attribute id are the pieces of information needed
product_id_tbl <- html %>% 
    html_nodes(".relatedProducts") %>%
    html_attr("id") %>%
    enframe(name = "position", value = "product_id") %>% #Simply makes the data into a dataframe from tibble
...
plan("multiprocess")
bike_ids_tbl <- bike_category_nids_tbl %>% mutate(bike_ids = future_map(url, get_bike_ids))

bike_ids_tbl

bike_ids_tbl <- bike_ids_tbl %>% select(-url) %>% unnest(bike_ids)
...
get_bike_features <- function(url) {
    ...
    tibble(features = features, description = feature_descriptions) %>%
...
bike_features_raw_tbl <- bike_ids_tbl %>% mutate(bike_features = map(url, get_bike_features))

# PROCESSED IN PARALLEL with furrr (5 minutes)
plan("multiprocess")
bike_features_raw_tbl <- bike_ids_tbl %>% mutate(bike_features = future_map(url, get_bike_features))
...
cannondale_bikes_2019_tbl <- bike_features_raw_tbl %>% select(-url) %>% unnest() %>%
    mutate(category = as_factor(category), product_family = as_factor(product_family),
           features = as_factor(features)) %>%
    spread(key = features, value = description) %>%
    rename(ProductFamily = product_family, WebsitePosition = position, ProductId = product_id) %>%
    rename_all(~str_remove_all(., " "))

cannondale_bikes_2019_tbl

cannondale_bikes_2019_tbl %>% write_csv("data/cannondale_bikes_2019.csv")
